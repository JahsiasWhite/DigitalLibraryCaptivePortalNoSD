...
const char *ssid = "Digital Library";  // FYI The SSID can't have a space in it.
...
void setUpWebserver(AsyncWebServer &server, const IPAddress &localIP) {
    ...
    int scriptPos = html.indexOf("<script>") + 8;
    String firstPart = html.substring(0, scriptPos);
    String secondPart = html.substring(scriptPos);
    String completePage = firstPart + "\n" + STORIES_JS + ";\n" + secondPart;
  ...
  // Serve book page with dynamic content
  server.on("/book", HTTP_GET, [](AsyncWebServerRequest *request) {
      String bookId = "";
      if (request->hasParam("id")) {
          bookId = request->getParam("id")->value();
      }
      
      String html = BOOK_PAGE;
      
      // Insert the books data and selected ID
      int scriptPos = html.indexOf("<script>") + 8;
      String firstPart = html.substring(0, scriptPos);
      String secondPart = html.substring(scriptPos);

      String completePage = "";
      if (bookId.equals("monet")) {
        completePage = firstPart + 
          "\n" + MONET + ";\n" +
          secondPart;
      } else if (bookId.equals("mozart")) {
        completePage = firstPart + 
          "\n" + MOZART + ";\n" +
          secondPart;
      } else {
  ...
  // server.on("/book", HTTP_ANY, [](AsyncWebServerRequest *request) {
  //       AsyncWebServerResponse *response = request->beginResponse(200, "text/html", BOOK_PAGE);
  //       response->addHeader("Cache-Control", "public,max-age=31536000");
  //       request->send(response);
  //   });
